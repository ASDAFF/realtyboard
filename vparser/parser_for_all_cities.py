# -*- coding: utf-8 -*-
import datetime, logging, json, os, re, time
...
from vparser.utils import  HTM, save_to_main_base, up_date_for_fast_pars_all_cities #,test_slando_proxy #CREDENTIALS, PROXY_PATH,
from vparser.utils import parse_int, save_to_buffer, CustomizationForParser
...
LOCALITY={
    'kiev'   :['http://kiev.ko.olx.ua/nedvizhimost',      8],
    'kharkov':['http://kharkov.kha.olx.ua/nedvizhimost', 20]
}
...
def save_to_base(grab, task, g2):
    ...
    room_tot = room()
    rent = type_rent()
    living_area_v = living_area_func()
    living_area_house_v = living_area_house_func()
    floor_v = floor_func()


    print grab.response.url
    print 'title = ', title.encode('utf-8')
    print 'text = ', text.encode('utf-8')+' '.join(grab.doc.select('//span[@class="show-map-link link gray cpointer"]').text().split(',')[1:]).encode('utf-8')
    ...
    if phones:
        board = save_to_main_base(
            ...
            room=room_tot,
            object_type=rent,
            living_area=living_area_v,
            living_area_house=living_area_house_v,
            floor=living_area_house_v
        )
...
class SiteStructureParser(CustomizationForParser):
    ...
    def task_link_on_page(self, grab, task):
        ...
        if len(h3_urls):
            ...
            for url in h3_urls:
                if up_date_for_fast_pars_all_cities(url[0], task.city, url[1]):
                    continue
                grab.change_proxy()
                grab.setup(url=url[0],
                           hammer_mode=True,
                           hammer_timeouts=HTM,
                           proxy_userpwd=self.CREDENTIALS,
                           user_agent=ua.random,
                ...
                yield Task('content', delay=4, grab=grab, cat=task.cat,
                           city=task.city, moroz=task.moroz, cost_uah=url[1])
        ...
        if status:
            if photo:
                for i, img in enumerate(photo):
                    ...
                    yield Task('img', delay=4, grab=grab, board=board, i=i, 
                               cat=task.cat, city=task.city, moroz=task.moroz)
...
def test_slandos():
    ...
    grab.setup(proxy='46.148.30.123:8080', 
               proxy_type='http', 
